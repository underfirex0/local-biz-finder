...
const TEAM_ID = "78f875c7-3cf6-4450-b2c5-4a2942b1e8a9";
...
const App: React.FC = () => {
  ...
  const loadLeads = async () => {
    ...
    const mapped: Lead[] =
      (data || []).map((row: any) => ({
        ...
        script: row.script || "",
        lastNiche: (row.last_niche || undefined) as any,
      })) || [];
  ...
  // CRM Actions (DATABASE)
  const saveLead = async (biz: BusinessInfo, category: string = "General") => {
    // simple duplicate guard
    if (leads.find((l) => l.name === biz.name && l.phone === biz.phone)) return;
  ...
  const saveAll = async () => {
    ...
    // Insert in parallel (safe for small counts)
    await Promise.all(allBusinesses.map((biz) => saveLead(biz, category)));
  };
  ...
  const handleGenerateScript = async (lead: Lead) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("leads")
        .update({ script, last_niche: selectedNiche })
        .eq("id", lead.id)
        .eq("team_id", TEAM_ID);
      ...
      setLeads((prev) =>
        prev.map((l) => (l.id === lead.id ? { ...l, script, lastNiche: selectedNiche } : l))
      );
      ...
      <header className="bg-white border-b border-slate-200 py-4 px-6 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <nav className="flex items-center bg-slate-100 p-1 rounded-xl">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${
                  activeTab === "finder" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500 hover:text-slate-700"
                }`}
              >
                1. Finder
              </button>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={logout}
              className="text-slate-600 border border-slate-200 bg-slate-50 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-slate-100 transition-all"
            >
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-left-4 duration-500">
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl flex items-center justify-between">
                ...
                {needsKeySelection && (
                  <button onClick={handleOpenKeySelector} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-xs font-bold">
                    Fix Key
                  </button>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  <div className="text-sm font-bold text-slate-600">
                    Found <span className="text-blue-600">{allBusinesses.length}</span> Moroccan results for{" "}
                    <span className="text-slate-900">"{currentSearchParams?.service}"</span> in{" "}
                    <span className="text-red-600">{currentSearchParams?.city}</span>
                  </div>
                  <button
                    onClick={saveAll}
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {allBusinesses.map((biz, idx) => {
                          const isSaved = !!leads.find((l) => l.name === biz.name && l.phone === biz.phone);
                          return (
                        ...
                        <div className="flex items-center gap-4">
                          <div
                            className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${
                              selectedLead?.id === lead.id ? "bg-red-500 text-white" : "bg-slate-100 text-slate-400"
                            }`}
                          >
                            {lead.name.charAt(0)}
                          </div>
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <p className={`text-[10px] ${selectedLead?.id === lead.id ? "text-red-100" : "text-slate-400"}`}>
                                {lead.phone}
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            ...
                            className={`p-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity ${
                              selectedLead?.id === lead.id ? "hover:bg-white/20 text-white" : "hover:bg-red-50 text-red-400"
                            }`}
                          >
                      ...
                      <div className="flex-1 flex flex-col space-y-6">
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap gap-2 pt-2">
                            <span className="text-[10px] bg-slate-100 px-2 py-1 rounded font-bold text-slate-600">
                              <i className="fas fa-tag mr-1"></i> {selectedLead.category}
                            </span>
                            <span className="text-[10px] bg-slate-100 px-2 py-1 rounded font-bold text-slate-600">
                              <i className="fas fa-star text-amber-500 mr-1"></i> {selectedLead.reviews}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {(["Web Design", "Social Media", "Google Ads", "SEO", "General Marketing"] as MarketingNiche[]).map(
                              (niche) => (
                                <button
                                  key={niche}
